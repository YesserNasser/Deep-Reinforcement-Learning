...
msf_stock = web.DataReader('MSFT', 'yahoo', start, end)
...
'''
Here we are going to de-trend the data. Otherwise MSFT and AAPL are on completely 
different scales and we could hadley expect the AI to learn how to trade apples and
oranges together. Additionally, we want the AI to learn the fundamentals of the stock
signal - buy if its going to rise. If we didnt remove the trend, maybe it'd only learn
to buy at the start and hold til the end since a general upwards trend happens.
'''
...
class TradingEnvironment():
    ...
    def __init__(self, starting_cash_mean=200., max_stride=5, series_length=208, starting_point=1, randomize_cash_std=0, \
        ...
        # states: five_day_window, five_day_window, starting_cash, portfolio_value, cur_apl_open, cur_msf_open
        self.state[0] = max(int(np.random.normal(self.starting_shares_mean, self.randomize_shares_std)), 0.) # aapl holdings?
        self.state[1] = max(int(np.random.normal(self.starting_shares_mean, self.randomize_shares_std)), 0.) # msft holdings?
